...
- **33 project entries** (`assets/entries/uid-*.json`, schema v6.1) in three layouts — `columns` (two-column case study), `flow` (typed-block long-form), and `gallery` (image-forward art showcase). All three share one top zone (hero thumbnail row + title/subtitle + role) and one tag model; they differ only in the content zone below.
