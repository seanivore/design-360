...
(function () {
  ...
  function editorHTML(p) {
    ...
    const dm = parseDims(eff("dimensions"));
    const seriesOpts = ["Art Movements", "Mid-Century Modern", "Barely Bauhaus", "Bauhaus Abstractions", "Andes Atelier", "Retro Tech", "— No collection —"];
        ...
        <div class="ed-cols">
          <div class="ed-main">
            <div class="ed-grid">
              ...
              ${f({ label: "Collection", tip: "Also called the series — an optional grouping in the shop.", value: p.series || "— No collection —", type: "select", options: seriesOpts, field: "series" })}
              ${f({ label: "Product", req: true, tip: "The kind of product — print, apparel, or merch.", value: p.product_type, type: "select", options: ["print", "apparel", "merch"], locked: published, field: "product_type" })}
            </div>
  ...
  function doPublish(id) {
    ...
    render(); P.logActivity("product.publish", "Published “" + (p.title || "a product") + "”"); P.toast("Published · live now", { kind: "live" });
  }
