...
(function () {
  ...
  async function openPreviewTab(p, id) {
    // DEMO FORK: query-param route + SAME-TAB navigation (the preview bar's Publish and the
    // browser Back both return to the admin; new-tab window.open dies in sandboxed previews).
    if (p.is_published && !p.draft && p.slug) { window.location.href = "product.html?slug=" + encodeURIComponent(p.slug); return; }
    try {
      const wrote = await persist(p, id);
      const url = p.preview_url || (p.slug && p.preview_token ? "product.html?slug=" + encodeURIComponent(p.slug) + "&preview=" + p.preview_token : null);
      if (!url) { P.toast(wrote ? "Preview unavailable — try saving again" : "Add a title first, then preview", { kind: "danger" }); return; }
  ...
  // DEMO FORK: no "Sold" tab. The demo sells restockable goods (posters, tees, caps — quantity can be
  // many), so a sold-out piece is a temporary stock state, not a lifecycle bucket. It stays under Live
  // labelled "Sold out" (its qty cell already renders in --sold). Everlastings' one-of-a-kind dioramas
  // keep the Sold tab in the live /admin. See DEMO_FORK_CHANGES.md.
  const TABS = [
  ...
  function inTab(p, t) {
    ...
    if (t === "live") return s === "live" || s === "edits" || s === "sold"; // sold-out stays under Live
  ...
  function copyLink(id) {
    ...
    const url = new URL("product.html?slug=" + encodeURIComponent(p.slug), location.href).href; // DEMO FORK: query-param route
