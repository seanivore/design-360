...
Entry pages have pre-generated HTML files in `_pages/{slug}.html`, created by `generate_manifest.py` as a Jekyll collection. Each file is a copy of `entry.html` with Jekyll front matter and SEO meta tags (og:title, og:description, og:image, twitter:card) baked in from the entry JSON. Jekyll outputs them at `/{slug}/` via the `permalink: /:basename/` config. This ensures social media crawlers always see correct meta tags.

```
User visits: august.style/saas-product-sale-features
  |
  v
Jekyll: _pages/saas-product-sale-features.html -> outputs /saas-product-sale-features/index.html -> serves it (HTTP 200)
  |
  v
...
Run `python3 generate_manifest.py` after adding, renaming, or removing any entry file. This also regenerates all `_pages/{slug}.html` files with updated SEO meta tags.
...
+-- _pages/{slug}.html                   Auto-generated entry pages with SEO meta tags (Jekyll collection)
|
+-- assets/
    +-- js/
