# v4.2.1 Implementation Plan — Entry Overhaul + Homepage Redesign + Media Collections
...
**Version**: v4.2.0 → v4.2.1
**Revision driven by**: 2026-05-27 cold-review subagent pass on v4.2.0 (18 findings: 5 blockers, 7 risks, 6 inconsistencies/vagueness). Closable findings folded; templates refreshed in lockstep; Wave 1 hero scope reshaped to reflect Sean's iteration concern (paired tool-assisted build, not static BUILD spec); block-type audit + working-dir audit + flat-grid audit added as ground truth.
**Status**: Draft — second revision. Most blockers resolved. Hand to another fresh cold-review (or steered-review focused on hero/asset-pipeline) next session for v4_2_2.
...
- `assets/docs/_entry_template.json` — **refreshed 2026-05-27 to v6.0** in lockstep with this revision. Deprecated fields (`mobile_img`, `gif`, flat `grid`, flat `slideshows`) removed. New fields (`layout`, `flow[]`, `main_media[]`, `bleed[]`, `bleed_slides{}`, `collection_preview`) populated with placeholder examples. WS-6 scope reduces to: align `JSON_ARCHITECTURE.md`, `ENTRY_SOP.md`, `README.md`, and `validate_v6.py` with this already-locked template.
- `assets/docs/_collection_template.json` — **refreshed 2026-05-27 to v6.0**, flat schema (no wrapper). UID format `uid-col-###`.
- `assets/docs/_item_template.json` — **refreshed 2026-05-27 to v6.0**, flat schema (no wrapper). UID format `uid-itm-###`. Includes `media_type` (`image | gif | video`) and `src`.
...
**Execution mode for the hero**: Sean's stated iteration concern (2026-05-27) is "getting the new homepage animation hero right since I was trying to apply a tutorial for new sites to this already built one." The hero is therefore scoped as a **paired tool-assisted build** following `.agent/CLAUDE_DESIGN_ANIM_SITE.md`, not a fully-pre-specified static BUILD. The orchestrator running TRACK_B starts at the existing `index.html` + `landing.css` (not a blank Vite project), iterates via the `claude.ai/design` loop with Sean in the room, and adapts the Prisma/motion-sites reference rather than ports it. The schema and Process-section data updates in this WS are static BUILD — only the hero markup/CSS/JS lives in the iteration loop.

**Scope**:

1. **New animated hero** — built via the iteration loop in `.agent/CLAUDE_DESIGN_ANIM_SITE.md` (claude.ai/design + Nano Banana 2 + Kling references). Target visual:
   ...
   - Rotating ABOUT-copy block (small-caps label + 1–2 sentence body) on the blurred half; copy pool seeded from v4_1_0's "My AI Focused Sell" bullets — concrete pool drafted in TRACK_B prep (Open Gap closed: Sean drafts 3–5 entries; rotation interval ~8s).
...
**Still open** (targets for v4_2_2 / steered review or in-track resolution):

| #   | Gap                                                | Type                  | Notes                                                                                                                                                                                                                                                                    |
| --- | -------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Hero Stats "Revamp" — kept or dropped from Wave 2? | Decision (post-jobs)  | Reframed: an interactive Projects/Roles/Skills chart that lives BELOW the homepage flow if pursued (not the hero). Wave 2/vNext. Not a Wave 1 blocker. Library candidates: ECharts, D3, Cytoscape, simple SVG. Data = `assets/entries/*.json` aggregated by tag overlap. |
| 2   | Cutout-letter font for cyberpunk hero              | Research (TRACK_B)    | Resolved live in the claude.ai/design session — Sean picks during the hero build. Walk in with candidates: Anton, Bebas Neue, Druk Wide, Inter Display.                                                                                                                  |
| 3   | `collection.html` + `media.html` UI wireframes     | Spec (Wave 2)         | Wave 2 / TRACK_D scope. Shopify-style all-products for collection; SEO-detail minimalism for media item.                                                                                                                                                                 |
| 4   | Sticky tag+embed column boundary                   | Spec (TRACK_A)        | Resolved inline: CSS `position: sticky; top: var(--space-md)` on the right column; terminates at `.entry-content-media` bottom; on short entries sticky has no visible effect. No new mechanism beyond standard CSS.                                                     |
| 6   | YouTube embed blurry-thumbnail root cause          | Research (Wave 2)     | Defer to TRACK_F. Showcase entries (the 3 new flow drafts) don't carry body-level YouTube embeds, so not Wave 1 blocking.                                                                                                                                                |
| 7   | `cdn_cleanup.py` delete behavior                   | Spec (TRACK_F)        | Default dry-run; `--apply` flag requires interactive confirmation. Wave 2.                                                                                                                                                                                               |
| 11  | Homepage `cta_section` copy                        | Decision (TRACK_B)    | Refresh copy in TRACK_B authoring with Sean; align with Phase A/B/C narrative; don't remove section.                                                                                                                                                                     |
| 12  | Item `tags[]` registry vs free-form                | Decision (Wave 2)     | Wave 2 / TRACK_D. Recommend free-form initially; constrain via `items-tags.json` registry later if filter UX demands it.                                                                                                                                                 |
| 14  | Achievement curation for the 3 new entries         | Spec (TRACK_C)        | TRACK_C decision with Sean: **1 achievement per entry** (3 total) for the homepage Achievements section, selected for job-application relevance. Each entry's full pool stays in the draft for future use.                                                               |
| 15  | Collection-preview placement on entry pages        | Spec (Wave 2)         | Wave 2 / TRACK_D. Default placement: after `bleed`/`bleed_slides`, before related-posts.                                                                                                                                                                                 |
| 17  | TRACK_B copy pool for rotating ABOUT block         | Spec (TRACK_B prep)   | Sean drafts 3–5 short lines (Prisma reference: ~1–2 sentences per rotation). Seed from v4_1_0's "My AI Focused Sell" bullets. Locked before TRACK_B starts.                                                                                                              |
| 18  | TRACK_B feature-card-1 video source                | Asset (TRACK_B prep)  | The 4-card grid below the hero has a video tile in card 1. Source clip needed — pipeline-process screen-record OR a Kling-generated motif. Locked before TRACK_B starts.                                                                                                 |
| 19  | `homepage-content.json` schema for new sections    | Spec (TRACK_B prep)   | Plan adds `quote{}`, `feature_cards[]`, `hero.copy_pool[]`. `renderShowcase` rewrite needs the schema sketched in TRACK_B opening before any code.                                                                                                                       |
| 20  | Process section data shape (link_slug vs filter)   | Spec (TRACK_B prep)   | Today `process.filter` is a tag filter; new 3-step uses direct `link_slug` references (one entry per phase). Shape shift from `{filter:{...}}` → `{link_slug:"..."}` for the three slots. Lock in TRACK_B prep.                                                          |
| 21  | Phase C copy → `homepage-content.json` mapping     | Spec (TRACK_C)        | Lift PHASE_C.md's positioning paragraphs into a `phase_c_copy` field used by `renderProcess` for the 3rd step (or feature-card 4 if cards align to phases). Lock in TRACK_C authoring with Sean.                                                                         |
...
- **v4_2_1 (2026-05-27)** — folded 2026-05-27 cold-review findings (18 surfaced, ~10 closed inline, remainder bounded). ✓ this file.
- **v4_2_2 (next session)** — **steered** review pass focused on the open Wave 1 surfaces. Suggested steered categories:
  - **TRACK_A schema/renderer lockstep**: do the new component CSS class names and entry-controller function rewrites land safely against `entry.html` as it stands? Is `populateImageGrids` clean-up of the legacy `grid[]` branch correct?
  - **TRACK_B hero iteration safety**: is the asset pre-flight (§ 4 #4) enough? What can go wrong in the claude.ai/design loop that the plan should pre-empt?
  - **`homepage-content.json` schema additions**: does the existing `landing-controller.js` rendering flow handle the new `quote`, `feature_cards`, `phase_c_copy`, `hero.copy_pool` shapes cleanly?
  - **WS-4 entry authoring**: are the flow drafts' 33/37/31 source files realistically going to produce 3 publishable v6-schema entries? Any media-volume blockers?
  - **Migration window** (§ 4 #5): is the temporary `legacy-slideshow` codepath actually as clean as claimed, or does it leak into other functions?
- **v4_2_3+**: alternate cold + steered until two consecutive passes return only nitpicks.
- **Stop condition**: nitpick-only → plan is exclusively executable → split into BUILD tracks. **Sean's call**: the plan is allowed to ship as TRACK_A + TRACK_B + TRACK_C with v4_2_2 if open Wave-1 gaps are all green even if Wave-2 gaps remain — Wave 2 has its own future planning rounds.
