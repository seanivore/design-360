...
def generate_entry_html(entries_dir: Path, project_root: Path) -> int:
    ...
    for json_file in json_files:
        ...
        # layout:"url" entries are pure redirect tiles — emit a tiny redirect page
        # (not the SPA entry clone) so a direct hit / shared link bounces to the URL.
        if data.get('layout') == 'url' and data.get('external_url'):
            url_attr = html.escape(data['external_url'])
            r_title = html.escape(data.get('seo_title', '') or data.get('title', '') or 'Redirecting…')
            redirect_html = (
                '---\n---\n'
                '<!DOCTYPE html>\n'
                '<!-- AUTO-GENERATED by generate_manifest.py — do not edit manually -->\n'
                '<html lang="en"><head><meta charset="utf-8">'
                f'<title>{r_title}</title>'
                f'<link rel="canonical" href="{url_attr}">'
                f'<meta http-equiv="refresh" content="0; url={url_attr}">'
                '<meta name="robots" content="noindex">'
                f'<script>location.replace({json.dumps(data["external_url"])})</script>'
                '</head><body>Redirecting to '
                f'<a href="{url_attr}">{url_attr}</a>…</body></html>'
            )
            pages_dir = project_root / '_pages'
            pages_dir.mkdir(parents=True, exist_ok=True)
            (pages_dir / f'{slug}.html').write_text(redirect_html, encoding='utf-8')
            generated += 1
            continue
